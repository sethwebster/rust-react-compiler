...
fn run() {
    ...
    for path in &entries {
        ...
        if !actual.contains("_c(") && expected.contains("_c(") {
            // Check if our output is the source passthrough
            let has_pragma = first.contains("@");
            missing_memo.push((name.to_string(), has_pragma, first.to_string()));
        }
    }

    println!("Missing memo: {} fixtures", missing_memo.len());
    println!("\nWith pragma:");
    for (name, _, pragma) in missing_memo.iter().filter(|(_, hp, _)| *hp) {
        println!("  {} -- {}", name, pragma);
    }
    println!("\nWithout pragma:");
    for (name, _, first) in missing_memo.iter().filter(|(_, hp, _)| !*hp).take(30) {
        println!("  {} -- first line: {}", name, &first[..first.len().min(80)]);
    }
}
