...
pub fn run_with_env(hir: &mut HIRFunction, env: &mut Environment) {
    ...
    // Second pass: absorb sentinel scopes into adjacent reactive (or sentinel) scopes
    // when ALL of the sentinel scope's declarations are last-used within the next scope.
    // This handles cases where the first pass couldn't merge because lvalue-check blocked
    // a prior pair but later merging created a new adjacent pair.
    //
    // The sentinel-A-before-B pattern: A has 0 deps (sentinel), and all of A's declared
    // identifiers are last-used before B.range.end. In this case, A's output is fully
    // consumed by B and A doesn't need to be independently memoized.
    //
    // Run to fixpoint (multiple passes until no more merges).
    loop {
        ...
        let mut i = 0;
        while i + 1 < scope_list.len() {
            let a_sid = scope_list[i];
            let b_sid = scope_list[i + 1];

            // Skip if either scope was already removed in this iteration.
            if !env.scopes.contains_key(&a_sid) || !env.scopes.contains_key(&b_sid) {
                i += 1;
                continue;
            }

            let a_end = env.scopes[&a_sid].range.end.0;
            let b_start = env.scopes[&b_sid].range.start.0;
            let b_end = env.scopes[&b_sid].range.end.0;

            // Only adjacent scopes (A.end <= B.start).
            if a_end > b_start { i += 1; continue; }

            let a_is_sentinel = env.scopes[&a_sid].dependencies.is_empty();
            if !a_is_sentinel { i += 1; continue; }

            // Require the gap between A and B to contain only safe instructions
            // (LoadLocal, Primitive, Const StoreLocal, etc.). If there are side-effect
            // instructions (CallExpression, useEffect call, etc.) in the gap, do NOT
            // absorb — those barriers make A and B semantically independent.
            let gap_safe_absorb = (a_end >= b_start) || {
                let mut safe = true;
                'outer: for (_, block) in &hir.body.blocks {
                    for instr in &block.instructions {
                        let iid = instr.id.0;
                        if iid >= a_end && iid < b_start {
                            if !is_safe_gap_instruction(&instr.value) {
                                safe = false;
                                break 'outer;
                            }
                        }
            ...
            if !gap_safe_absorb { i += 1; continue; }

            // Only absorb if ALL of scope A's declarations are always-invalidating.
            // If scope A contains declarations that are potentially stable (e.g., function
            // call results like `someObj()`), it may produce the same reference across renders
            // and should remain an independent memoization slot. Always-invalidating values
            // (like `[]` or `{}`) always create new allocations and provide no benefit from
            // being independently memoized — merging them is always safe.
            let a_all_always_inv: bool = env.scopes[&a_sid].declarations.keys().all(|&decl_id| {
            ...
            if !a_all_always_inv { i += 1; continue; }

            // TS Case 2 analog: B's deps must all be direct (no property path) references
            // to A's always-invalidating declarations.  If B depends on something that is NOT
            // declared by A (like `props.value`), then A's outputs are independently stable
            // across re-renders of B and MUST NOT be absorbed — they belong in their own
            // sentinel slot.
            //
            // Example where absorption is WRONG:
            //   Scope A (sentinel): {} and []
            //   Scope B (dep: props.value): [{}, [], props.value]
            //   B's deps include props.value which is NOT declared by A → do NOT absorb.
            //
            // Example where absorption is RIGHT (only theoretical — handled by first-pass Case 2):
            //   Scope A (sentinel, declares t0=[]): t0
            //   Scope B (dep: t0): [t0, ...]
            //   B's dep `t0` is declared by A with path.is_empty() → absorb (same as Case 2).
            let a_decl_ids: std::collections::HashSet<IdentifierId> =
                env.scopes[&a_sid].declarations.keys().copied().collect();
            let b_deps_all_from_a = !env.scopes[&b_sid].dependencies.is_empty()
                && env.scopes[&b_sid].dependencies.iter().all(|dep| {
                    dep.path.is_empty()
                        && (a_decl_ids.contains(&dep.place.identifier)
                            || store_local_value
                                .get(&dep.place.identifier)
                                .map(|&val_id| a_decl_ids.contains(&val_id))
                                .unwrap_or(false))
                });
            if !b_deps_all_from_a { i += 1; continue; }

            // Collect named binding identifiers that are StoreLocal'd from scope A's declarations.
            let mut a_binding_ids: std::collections::HashSet<IdentifierId> = a_decl_ids.clone();
            for (_, block) in &hir.body.blocks {
                for instr in &block.instructions {
                    let iid = instr.id.0;
                    if iid >= a_end && iid < b_start {
                        if let InstructionValue::StoreLocal { lvalue, value, .. } = &instr.value {
                            if a_decl_ids.contains(&value.identifier) {
                                a_binding_ids.insert(lvalue.place.identifier);
                            }
                        }
                    }
                }
            }
            let a_decls_consumed: bool = a_binding_ids.iter().all(|&id| {
                // Use <= b_end: scope output bindings are always available just after
                // scope B (the codegen lifts them as `let x; if (changed) { x = ...; }`),
                // so using them at exactly b_end (the first instruction just outside B)
                // is safe. This allows cases like `return a` where `a` is a scope output.
                last_use(id) <= b_end
            });

            if a_decls_consumed {
                if std::env::var("RC_DEBUG").is_ok() {
                    eprintln!("[merge_absorb] absorbing sentinel scope {:?} into {:?}", a_sid.0, b_sid.0);
                }
                merge_scopes_a_into_b(env, a_sid, b_sid, &store_local_value);
                // Remove a_sid from scope_list to keep indices valid.
                scope_list.remove(i);
                merged_any = true;
                // Don't advance i — the new b_sid might be absorbable by the next scope.
            } else {
                i += 1;
            }
        }
